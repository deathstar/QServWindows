...
#include <errno.h>
    ...
    void QServ::getLocation(clientinfo *ci) {
        ...
        */
        ...
        if(!strcmp(ip,localhost_s1) || isPartOf(ip,localhost_s2) || isPartOf(ip,localhost_s3)) location = (char*)"localhost";
        else location = (char*)cgip(ip).c_str();
        ...
        if(strlen(ip) > 2) {
            ...
            //localhost exclusion
            } else if(!strcmp(ip,localhost_s1) || isPartOf(ip,localhost_s2) || isPartOf(ip,localhost_s3)) {
                type = 1;
                typeconsole = 1;
            ...
            //found geoip data
                ...
                type = 2;
                typeconsole = 2;
                sprintf(lmsg, "%s %s", types[type], (char*)cgip(ip).c_str());
                sprintf(pmsg, "%s%s", typesconsole[typeconsole], (char*)cgip(ip).c_str());
            }
